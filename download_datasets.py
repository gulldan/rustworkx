...
def unzip_file(gz_path, txt_path):
    """Unzip a .gz file if the target .txt file doesn't exist."""
    if not os.path.exists(txt_path):
        if os.path.exists(gz_path):
            ...
            print(f"Warning: Cannot unzip, {gz_path} not found.")
    else:
        print(f"{txt_path} already exists (no need to unzip).")


def save_football_network():
    """Download and save the football network in GML format."""
    os.makedirs("datasets", exist_ok=True)
    output_path = "datasets/football.gml"
    if not os.path.exists(output_path):
        # Используем репозиторий vlivashkin/community-graphs
        download_from_github("gml_graphs", "football.gml", output_path)
            ...
            try:
                ...
                with open(content_file) as f:
                    for line in f:
                        ...
                        G.add_node(
                            node_id, label=label, features=features, value=class_mapping.get(label, 0)
                        )
...
def save_orkut_network():
    """Download Orkut dataset files (edge list and communities) from SNAP and unzip them."""
    print("\nProcessing Orkut dataset...")
    os.makedirs("datasets", exist_ok=True)
    edge_gz = "datasets/com-orkut.ungraph.txt.gz"
    edge_txt = "datasets/com-orkut.ungraph.txt"
    cmty_gz = "datasets/com-orkut.all.cmty.txt.gz"
    cmty_txt = "datasets/com-orkut.all.cmty.txt"

    # Edge list
    download_file("https://snap.stanford.edu/data/bigdata/communities/com-orkut.ungraph.txt.gz", edge_gz)
    unzip_file(edge_gz, edge_txt)

    # Communities
    download_file(
        "https://snap.stanford.edu/data/bigdata/communities/com-orkut.all.cmty.txt.gz", cmty_gz
    )
    unzip_file(cmty_gz, cmty_txt)

    print("Orkut dataset files downloaded and unzipped.")


def save_livejournal_network():
    """Download LiveJournal dataset files (edge list and communities) from SNAP and unzip them."""
    print("\nProcessing LiveJournal dataset...")
    os.makedirs("datasets", exist_ok=True)
    edge_gz = "datasets/com-lj.ungraph.txt.gz"
    edge_txt = "datasets/com-lj.ungraph.txt"
    cmty_gz = "datasets/com-lj.all.cmty.txt.gz"
    cmty_txt = "datasets/com-lj.all.cmty.txt"

    # Edge list
    download_file("https://snap.stanford.edu/data/bigdata/communities/com-lj.ungraph.txt.gz", edge_gz)
    unzip_file(edge_gz, edge_txt)

    # Communities
    download_file("https://snap.stanford.edu/data/bigdata/communities/com-lj.all.cmty.txt.gz", cmty_gz)
    unzip_file(cmty_gz, cmty_txt)

    print("LiveJournal dataset files downloaded and unzipped.")
